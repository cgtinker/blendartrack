from .helper import KeyframeAssistent, JsonDecoder, WorldToCameraScreen
import importlib
...
def init_screen_to_world_model(json_data, title):
    ...
    for data in json_data[title]:
        x, y, z = JsonDecoder.get_vert_data(data['screenPos'])
        tx, ty, tz = JsonDecoder.get_vert_data(data['objPos'])
        frame = data['frame']
